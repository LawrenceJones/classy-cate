...
parseExercises = ($, $page) ->
  ...
  extract_term_title = ($page) ->
    $page.find('tr').eq(0).find('h1').eq(0).text()
  ...
  extract_start_end_dates = ($full_table, years) ->
    ...
    [first_month, others..., last_month] = extract_months $full_table.find('tr').eq(0)

    year = if first_month < 9 then years[1] else years[0]

    day_headers = $full_table.find('tr').eq(2).find('th')

    col_buf = 0
    col_buf += 1 while $(day_headers[col_buf]).is(":empty")

    [first_day, others..., last_day] = extract_days $full_table.find('tr').eq(2)
  ...
  extract_module_exercise_data = ($full_table) ->
  ...
  term_title = extract_term_title $page
  $timetable = ($ tb for tb in $page.find('table') when $(tb).attr('border') == "0")[0]
  dates = extract_start_end_dates $timetable, extract_academic_years $page   # WRONG
...
exports.getExercises = (req, res) ->
  options =
    url: 'https://cate.doc.ic.ac.uk/timetable.cgi?period=3&class=c2&keyt=2013%3Anone%3Anone%3Almj112'
    auth: req.creds
